...
def NUTS(
    ...
    def sample_device_chain(states_device, params_device, key_device):
        ...
        def run_chain(state, params, key_chain):
            inv_mass_matrix = params["inverse_mass_matrix"]
            ...
            def one_step(carry, key_inner):
                state = carry
                state, info = kernel(key_inner, state)
                return state, (state.position, info.num_integration_steps)

            inner_keys = jax.random.split(key_chain, num_results)
            _, (samples, num_grads) = jax.lax.scan(one_step, state, inner_keys)
            return samples, num_grads

        samples, num_grads = vmap(run_chain)(states_device, params_device, keys)
        return samples, num_grads

    sample_device_chain_pmap = pmap(sample_device_chain, devices=local_devices)
    local_samples, local_num_grads = sample_device_chain_pmap(local_warmup_states, local_warmup_params, local_device_keys_np)

    samples = mhu.process_allgather(local_samples)
    all_num_grads = mhu.process_allgather(local_num_grads)

    end = time.time()
    if process_idx == 0: print(f'Sampling took {(end - start):.1f}s')

    samples = jnp.reshape(samples, (-1, num_results, dim))
    total_grad_calls = int(jnp.sum(all_num_grads))

    if process_idx == 0:
        ess = blackjax.diagnostics.effective_sample_size(samples, chain_axis=0, sample_axis=1)
        min_ess = float(jnp.min(ess))
        mean_ess = float(jnp.mean(ess))
        print(f'Total grad calls: {total_grad_calls}')
        print(f'Min ESS: {min_ess:.1f}, Mean ESS: {mean_ess:.1f}')
        print(f'Grads/ESS (min): {total_grad_calls / min_ess:.1f}')

    return samples, total_grad_calls
